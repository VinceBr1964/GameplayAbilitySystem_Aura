...
void UAuraAbilitySystemComponent::ServerEquipAbility_Implementation(const FGameplayTag& AbilityTag, const FGameplayTag& Slot)
{
	if (FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		...
		if (bStatusValid)
		{
			...
			// Handle activation and deacivation for passive abilities
			...
			if (!SlotIsEmpty(Slot)) // There is an ability in this slot already.  Deactivate and clear it's slot
			{
				...
				if (SpecWithSlot)
				{
					// are the two abilities the same if so return early
			...
			if (!AbilityHasSlot(*AbilitySpec, Slot)) // Ability doesn't yet have a slot.  In other words, it's not active.
			{
